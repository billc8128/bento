...
import type { SessionBackend } from "../collaboration/collaboration-service"
import { UiCommandBridge } from "../collaboration/ui-command-bridge"
import { localHarnessExecutable } from "../runtime/harness-runtime"
import { resolvePiRpcEntry } from "../runtime/pi-rpc-entry"
...
describe("AppRuntimeHost", () => {
  it("一个 session endpoint 同时服务 HTTP、stdio relay 与 Browser runtime", async () => {
    ...
    const extensionSource = pathToFileURL(path.join(process.cwd(), "electron/apps/pi-mcp-extension.mjs")).href
