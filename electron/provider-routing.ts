...
export class ProviderRoutingService {
  ...
  /**
   * Pi 读取隔离 models.json；密钥只通过进程环境注入，绝不写入配置文件。
   * 每个 Provider 独占目录，因此 models.json 内可统一使用稳定 provider id `bento`。
   */
  piProviderEnv(providerId: string): { env: Record<string, string>; strip: string[] } {
    const config = this.providers().getProviderConfig(providerId)
    if (!config) throw new Error(`供应商不存在: ${providerId}`)
    const runtime = config.runtimes.pi
    if (!runtime) throw new Error(`供应商 ${config.name} 未配置 Pi runtime`)
    if (config.auth.method === "oauth") throw new Error("Pi 暂不支持该供应商的 OAuth 路由")
    const apiKey = config.auth.method === "none"
      ? "local"
      : this.providers().readKey(providerId, "pi")
    if (!apiKey) throw new Error(`供应商 ${config.name} 缺少 API key,请到设置页补填`)
    const api = runtime.wireProtocol === "openai-chat"
      ? "openai-completions"
      : runtime.wireProtocol
    const headers = { ...(runtime.auth?.inference.fixedHeaders ?? {}) }
    const auth = runtime.auth?.inference
    if (auth && !["authorization", "x-api-key"].includes(auth.header.toLowerCase())) {
      headers[auth.header] = `${auth.prefix ?? ""}$BENTO_PROVIDER_KEY`
    }
    const dir = path.join(this.userDataDir, "providers", `pi-${providerId}`)
    fs.mkdirSync(dir, { recursive: true })
    fs.writeFileSync(path.join(dir, "models.json"), JSON.stringify({
      providers: {
        bento: {
          ...
          api,
          apiKey: "$BENTO_PROVIDER_KEY",
          ...(Object.keys(headers).length > 0 ? { headers } : {}),
          models: runtime.models.filter((model) => model.enabled !== false).map((model) => ({
            id: model.id,
            name: model.name,
    ...
    return {
      env: { PI_CODING_AGENT_DIR: dir, BENTO_PROVIDER_KEY: apiKey },
      strip: ["PI_CODING_AGENT_DIR", "BENTO_PROVIDER_KEY"],
    }
  }

  configuredHarnessEnv(
    sessionKey: string,
    providerId: string,
    harnessId: "kimi" | "opencode" | "omp" | "hermes",
    wireModelId: string,
  ): { env: Record<string, string>; strip: string[] } {
    const config = this.providers().getProviderConfig(providerId)
    if (!config) throw new Error(`供应商不存在: ${providerId}`)
    ...
    const apiKey = config.auth.method === "none"
      ...
      : config.auth.method === "apiKey"
        ? this.providers().readKey(providerId, harnessId)
        : null
    if (!apiKey) throw new Error(`供应商 ${config.name} 缺少 ${harnessId} API key`)
    const modelId = wireModelId.replace(/^bento\//, "")

    if (harnessId === "kimi") {
      const providerType = runtime.wireProtocol === "anthropic-messages"
        ? "anthropic"
        : runtime.wireProtocol === "openai-responses" ? "openai_responses" : "openai"
      return {
        env: {
          KIMI_MODEL_NAME: modelId,
          KIMI_MODEL_DISPLAY_NAME: modelId,
          KIMI_MODEL_API_KEY: apiKey,
          KIMI_MODEL_BASE_URL: runtime.baseUrl,
    ...
    const dir = path.join(this.userDataDir, "providers", `${harnessId}-${sessionKey}`)
    fs.mkdirSync(dir, { recursive: true })
    if (harnessId === "opencode") {
      const providerPackage = runtime.wireProtocol === "anthropic-messages"
        ? "@ai-sdk/anthropic"
      ...
      const provider = {
        bento: {
          npm: providerPackage,
          name: config.name,
          options: { baseURL: runtime.baseUrl, apiKey: "{env:BENTO_PROVIDER_KEY}" },
          models: { [modelId]: { name: modelId } },
        },
      }
      const file = path.join(dir, "opencode.json")
      fs.writeFileSync(file, JSON.stringify({ model: `bento/${modelId}`, provider }, null, 2))
      return {
        env: { OPENCODE_CONFIG: file, BENTO_PROVIDER_KEY: apiKey },
        strip: ["OPENCODE_CONFIG", "BENTO_PROVIDER_KEY"],
      }
    }
    ...
    if (harnessId === "omp") {
      ...
      fs.writeFileSync(path.join(agentDir, "models.json"), JSON.stringify({
        providers: {
          bento: {
            name: config.name,
            baseUrl: runtime.baseUrl,
            api: runtime.wireProtocol === "openai-chat"
              ? "openai-completions"
              : runtime.wireProtocol,
            apiKey: "$BENTO_PROVIDER_KEY",
      ...
      return {
        env: { OMP_HOME: dir, BENTO_PROVIDER_KEY: apiKey },
        strip: ["OMP_HOME", "BENTO_PROVIDER_KEY"],
      }
    }

    const apiMode = runtime.wireProtocol === "anthropic-messages"
      ? "anthropic_messages"
      : runtime.wireProtocol === "openai-responses" ? "codex_responses" : "chat_completions"
    fs.writeFileSync(path.join(dir, "config.yaml"), JSON.stringify({
      model: {
        provider: "custom",
        default: modelId,
        base_url: runtime.baseUrl,
        api_key: "${BENTO_PROVIDER_KEY}",
        api_mode: apiMode,
      },
    }, null, 2))
    return {
      env: {
        ...
        BENTO_PROVIDER_KEY: apiKey,
      },
      strip: ["HERMES_HOME", "HERMES_INFERENCE_MODEL", "BENTO_PROVIDER_KEY"],
    }
  }

  disposeConfiguredHarnessHome(sessionKey: string): void {
    for (const harnessId of ["opencode", "omp", "hermes"]) {
      fs.rmSync(path.join(this.userDataDir, "providers", `${harnessId}-${sessionKey}`), {
        recursive: true,
        force: true,
      })
    }
